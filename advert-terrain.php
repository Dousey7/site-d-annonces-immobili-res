...
<div class="container" style="background-color: darksalmon;">
    <div class="row">
        <?php
            $req = $db->prepare("SELECT id, titre, description, auteur_id, image_url, localisation, statut, DATE_FORMAT(date_creation, '%d/%m/%Y à %H:%i:%S') AS date_creation_format FROM annonce WHERE titre = 'terrain' ORDER BY date_creation DESC");
            $req->execute();
